observeEvent(input$createIssue00, {
  queryCreateIssue01 <- reactive({dbGetQuery(db, paste0('SELECT uniqueIdentifier FROM tbl', input$createIssueSelectProject))$uniqueIdentifier})
...
observeEvent(input$createIssue04, {
  queryCreateIssue02 <- reactive({dbGetQuery(db, paste0('SELECT * FROM tbl', input$createIssueSelectProject))})
  tmp01 <- ifelse(nrow(queryCreateIssue02()) < 1, 1, nrow(queryCreateIssue02()) + 1)
  tmp02 <- Sys.Date()
  queryCreateIssue03 <- reactive({dbSendQuery(db, paste0('INSERT INTO tbl', input$createIssueSelectProject, ' (uniqueIdentifier, issueStatus, issueDescription, issueOpeningDate) VALUES ('", input$createIssue01, " (#", tmp01, ")', '", input$createIssue03, "', '", input$createIssue02, "', '", tmp02, "')"))})
  queryCreateIssue03()
